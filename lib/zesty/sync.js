          ...
          if (settings && Array.isArray(settings.data)) {
            ...
            if (site_protocol) {
              await fetch(
                ...
                .catch((err) => {
                  throw new Error(
                    `Failed updating instance "site_protocol" settings`,
                  );
                });
            }
...
async function createFiles(config) {
  ...
  for (const model of gqlJSON.models) {
    ...
    if (
      config.options?.model_ignore_list &&
      config.options.model_ignore_list.find(
        (modelZUID) => modelZUID == model.zuid,
      ) !== undefined
    ) {
      steps.push(
        Promise.resolve(
          `Ignored: ${model.label} [${model.zuid}] is set to be ignored in zesty.config.json`,
        ),
      );
      continue;
    }
